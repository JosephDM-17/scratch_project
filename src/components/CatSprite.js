...
export default function CatSprite({ sprite, manager }) {
  const [dragging, setDragging] = useState(false);
const isSelected = sprite.id === manager.selectedId;

  const style = {
  position: "absolute",
  transform: `translate(${sprite.x}px, ${sprite.y}px) rotate(${sprite.rot}deg)`,
  width: "95px",
  height: "100px",
  ...
  const onMouseDown = (e) => {
   ...
   const deleteSprite = () => {
    manager.deleteSprite(sprite.id);
  };

  return (
    <div style={style} onMouseDown={onMouseDown} onClick={() => manager.selectSprite(sprite.id)}>
      {/* SVG (your full cat) */}
      {isSelected && (
        <div
